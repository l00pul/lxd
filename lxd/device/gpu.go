...
func gpuValidationRules(requiredFields []string, optionalFields []string) map[string]func(value string) error {
	...
	defaultValidators := map[string]func(value string) error{
		...
		//
		// - `nvidia.com/gpu=0` : this instructs LXD to operate a discrete GPU (dGPU) passthrough of brand NVIDIA with the first discovered GPU on your system. You can use the `nvidia-smi` tool on your host to know which identifier to use.
		// - `nvidia.com/igpu=0` : this instructs LXD to operate an integrated GPU (iGPU) passthrough of brand NVIDIA with the first discovered iGPU on your system. An iGPU won't be detected by the `nvidia-smi` tool, so we encourage the user to use the `nvidia-ctk` tool through `nvidia-ctk cdi generate` to list the available iGPU devices.
...
func constructCDIMountList(nativeDevices map[string]map[string]string, cdiID cdi.ID, mounts []*specs.Mount, instPrivileged bool) ([]cdi.SymlinkEntry, error) {
	...
	if cdiID.Vendor == cdi.Nvidia && cdiID.Class == cdi.IGPU {
		tegraCSVFilesCandidates := cdi.DefaultNvidiaTegraCSVFiles(rootPath)
...
func constructCDIHook(hook *specs.Hook, hooks *cdi.Hooks) error {
	...
	case "create-symlinks":
		if len(hook.Args) < 5 {
			return fmt.Errorf("CDI 'create-symlinks' hook has not enough arguments: %v", hook.Args)
		}
		...
		for i := 4; i < len(hook.Args); i += 2 {
			...
			// path of the `Target`
			hooks.Symlinks = append(hooks.Symlinks, cdi.SymlinkEntry{Target: strings.TrimPrefix(entry[0], rootPath), Link: strings.TrimPrefix(entry[1], rootPath)})
		}

	case "update-ldcache":
		// The `update-ldcache` hook can be used without any arguments, but in our case we don't need it.
		if len(hook.Args) < 5 {
			logger.Warnf("CDI 'update-ldcache' hook has not enough arguments: %v", hook.Args)
			return nil
		}
	...
	case "chmod":
		...
		logger.Debug("CDI 'chmod' hook is ignored")
		return nil
	default:
...
func resolveTargetRelativeToLink(link, target string) (string, error) {
	if !filepath.IsAbs(link) {
		return "", fmt.Errorf("link must be an absolute path: %s (target: %s)", link, target)
	}
...
// linuxArchTriplet matches the architecture triplet of a Linux system.
// e.g. x86_64-linux-gnu, aarch64-linux-gnueabihf, etc.
var linuxArchTriplet = regexp.MustCompile(`([\w-]+)-linux-([\w-]+)`)

// getArchTripletFromPaths returns the architecture triplet from a list of paths.
func getArchTripletFromPaths(paths []string) (string, error) {
	for _, path := range paths {
		matches := linuxArchTriplet.FindStringSubmatch(path)
		if len(matches) > 2 {
			return matches[1] + "-linux-" + matches[2], nil
		}
	}

	return "", fmt.Errorf("could not determine architecture triplet from paths")
}

// generateSymlinksAndLinkerCmd generates the shell command to be executed inside the container
// to create all the necessary symlinks and update the linker cache.
func generateSymlinksAndLinkerCmd(hooks *cdi.Hooks) (string, error) {
	cmd := ""
	...
	for i, hook := range hooks.Symlinks {
		...
		cmd += fmt.Sprintf("mkdir -p %s; ln -s %s %s", filepath.Dir(hook.Link), absTarget, hook.Link)
		if i+1 < l {
			cmd += "; "
		}
	}
	...
	if l > 0 {
		cmd += "; "
		archTriplet, err := getArchTripletFromPaths(hooks.LDCacheUpdates)
		if err != nil {
			return "", err
		}

		ldConfFile := fmt.Sprintf("/etc/ld.so.conf.d/%s.conf", archTriplet)
		for i, update := range hooks.LDCacheUpdates {
			cmd += fmt.Sprintf("echo %s >> %s", update, ldConfFile)
			if i+1 < l {
				cmd += "; "
			}
		}
	...
	cmd += "; rm /etc/ld.so.cache; ldconfig"
	return cmd, nil
}
...
func constructCDIDevices(inst instance.Instance, cdiID cdi.ID) (map[string]map[string]string, *cdi.Hooks, error) {
	...
	if len(cdiName) == 1 {
		...
		targetName = cdiName[0]
	} else if len(cdiName) == 2 && cdiID.Class == cdi.MIG {
		targetName = fmt.Sprintf("%s:%s", cdiName[0], cdiName[1])
	} else {
		return nil, nil, fmt.Errorf("invalid CDI locator %v", cdiName)
	}
...
func startWithCDISpec(s *state.State, inst instance.Instance, cdiID cdi.ID, runConf *config.RunConfig) error {
	...
	cmd, err := generateSymlinksAndLinkerCmd(hooks)
	if err != nil {
		return err
	}
